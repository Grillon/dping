...
#-dlcnis : nbr_arg_single_ping=4 nbr_arg_single_ping_option=0à2
...
if [ "$nbr_arg_single_ping" -eq 4 ];then
	...
	custom_ping ${ip_dest} ${nbr_iteration} ${taille_packets} ${lan} ${capture}>${sortie_ping}
	analyse_ping >>${COMPTE_RENDU}
	if [ $capture -eq 1 ];then
		arret_job_capture $job_tcpdump
	fi
	exit 0;
if [ "$nbr_arg_transfert" -eq 2 ];then
	test_bp $user_at_cible_scp $lan 2>>$RESULTAT_ERR 1>>$RESULTAT_BPP
	sed -ni "/maintenant/p" $RESULTAT_ERR
	exit 0;
else 
	erreur $KO "arguments $arguments incorrects" $ESTOP
fi
if [ "$capture" -eq 1 ];then
